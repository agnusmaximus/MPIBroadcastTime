...
int main(void) {
    ...
    char *data, *worker_data;
    if (proc_id == 0) {
	data = (char *)malloc(sizeof(char) * n_procs * SCATTER_ARRAY_SZ);
	for (int i = 0; i < n_procs * SCATTER_ARRAY_SZ; i++)
	    data[i] = (char)(i % 256);
    }
    worker_data = (char *)malloc(sizeof(char) * SCATTER_ARRAY_SZ);
    ...
    //Scatter to workers
    MPI_Scatter(data, SCATTER_ARRAY_SZ, MPI_CHARACTER, worker_data, SCATTER_ARRAY_SZ, MPI_CHARACTER, 0, MPI_COMM_WORLD);
